...
static void onOpen(___notnull descriptor_event_subscription_t * subscription, uint32_t type, event_subscription_event_t * node) {
    ...
    network_netlink_req(vpn_client_app_netlink_descriptor_get(), network_netlink_message_ipaddr_add_gen(AF_INET, addr, 24, "tun0"), nil);
    network_netlink_req(vpn_client_app_netlink_descriptor_get(), network_netlink_message_iplink_setup_gen("tun0"), nil);
    network_netlink_req(vpn_client_app_netlink_descriptor_get(), network_netlink_message_iprule_add_gen(network_netlink_table_main_mark, network_netlink_table_main_priority, network_netlink_table_main_id), nil);
    network_netlink_req(vpn_client_app_netlink_descriptor_get(), network_netlink_message_iprule_add_gen(network_netlink_table_tun_mark, network_netlink_table_tun_priority, network_netlink_table_tun_id), nil);
    network_netlink_req(vpn_client_app_netlink_descriptor_get(), network_netlink_message_iproute_prepend_gen(all, 0, addr, network_netlink_table_tun_id), nil);
    // DEFAULT ROUTE GET
}
