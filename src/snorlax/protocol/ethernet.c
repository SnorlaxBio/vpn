...
extern void snorlax_protocol_ethernet_debug(FILE * stream, const uint8_t * frame) {
    ...
    switch(type) {
        ...
        case snorlax_protocol_ethernet_type_lldp:   snorlax_implement("Link Layer Discovery Protocol");     break;
        default:                                    snorlax_implement("Unknown Protocol => %04x", type);    break;
    }
}
#endif // SNORLAX_DEBUG
